...
x_train = load_mnist('mnist')[0]
data_provider = DataProvider(x_train.astype(np.float32))

noise_prior = torch.distributions.uniform.Uniform(0, 1)

G = MLPGenerator(noise_prior, input_dim=784, output_dim=784, hidden_dim=128, n_hidden_layers=3, lr=1e-3)
...
m = 256 # number of samples per minibatch
...
# plotting!
x_gen = gan.G.sample(1, training=False)
plt.imshow(x_gen[0].detach().numpy().reshape(28, 28))
